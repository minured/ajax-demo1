...
var server = http.createServer(function(request, response) {
  ...
  if (path === "/index.html") {
    ...
    //这里fs读取返回的是一个对象，不是string
    let indexStr = fs.readFileSync("public/index.html").toString()
    console.log(typeof indexStr)
    page1 = fs.readFileSync("db/page1.json")

    let array = JSON.parse(page1)
    let result = array.map(item => `<li>${item.id}</li>`).join("")
    console.log(result)
    indexStr = indexStr.replace("{{ page1 }}", `<ul id="pageUl">${result}</ul>`)
